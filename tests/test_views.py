...
def test_cards(df6):
    assert cards(df6) == [
        {"last_digits": "5091", "total_spent": 260.73, "cashback": 2.61},
...
def test_service_amount_by_category(df6, date_time):
    assert service_amount_by_category(df6, date_time) == [
        {"category": "Переводы", "amount": 90000.0},
...
def test_service_transfers_and_cash(df6, date_time):
    assert service_transfers_and_cash(df6, date_time) == [
        {"category": "Переводы", "amount": 90000},
